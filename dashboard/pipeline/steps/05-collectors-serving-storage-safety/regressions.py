...
""",
    ...
    # ================================================== §3.5 disk
    ("S1 the divisor becomes 10^9, so no figure matches the `df -h` §6.6 says to check against",
    ...
    ("S2 `used` is computed from the free blocks rather than the used ones",
     ...
     STORAGE),
    ("S3 a zero block size passes, and a 931 GiB volume renders `0.0 / 0.0 GiB`",
...
""",
    ...
    ("T2 the port is minted from Number() rather than parsed and branded",
     ...
     "types"),
    ("T3 Filesystem loses the GiB brand, so an unbranded byte count could be assigned to it",
