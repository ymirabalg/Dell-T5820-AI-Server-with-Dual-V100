...
## 8. Spec gaps and open owner questions — ⚠⚠ **12d raises SEVEN, three of them ⚠⚠: `12d-Q1` is an OVERRIDE of an explicit `SPEC.md` table row (not a silence — the build mis-classified it), `12d-Q2` is the copy for the new card form, and `12d-Q6` is a contradiction between two panels that 12d's own ruling created**; 12c raises SIX, two of them ⚠⚠: `12c-Q1` is what a GPU card may SAY when the serving list was read in part, and `12c-Q2` asks whether an enumeration may be read in PART at all for §9's purposes**; 12b's six stand (`12b-Q1` is a line of `SPEC.md` §6.2 that states the RETIRED join as the rule); 12a's five (`12a-Q2` is the other false sentence); step 11's three (`11-Q1`…`11-Q3`) were RULED on 2026-09-11 and are BUILT by 11b; 11b's five are wording; 10h's six; 10g's four still open; 10f's one; 10e's eight

### ⚠⚠ NEW — 12d's seven, 2026-09-22. **One is an OVERRIDE of an explicit `SPEC.md` table row rather than a silence, one is a contradiction 12d's own ruling created between two panels, and one asks whether staleness needs a horizon now that this loop has widened it.**

Full statements in `steps/12-deploy/12d-build.md` §5 and `steps/12-deploy/12d-reconciliation.md` §1
and §2. ⚠ The ⚠⚠ rows were **re-classified by the reconcile phase**; the build's own wording for
`12d-Q1` called an override a silence and `12d-Q3`'s reason quoted a sentence `SPEC.md` does not
contain.

| id | question | what stands today | Owner |
|---|---|---|---|
| ⚠⚠ **`12d-Q1`** | ⚠⚠ **NOT a silence — an OVERRIDE.** §3.4's `gpus` table assigns `null` the render *"invariant 1: an em dash, and the `errors[]` entry says why"* **unconditionally**. When the serving list was ALSO cut, the card shows **`list not fully read`** instead. §6.2 is silent about the precedence; §3.4 is not, and the built answer overrides it | **`incomplete` wins**, pinned at two layers (`observations.test.ts`, and `wire.test.ts` end to end from raw bytes), with `12d-O6` as its mutation. ⚠ The argument for it is about **proximity**, not presence — both explanations do reach SERVING, one on the row through `errors[].instance`, one under the rows through `PanelNotes` — which is a weaker basis for overriding an explicit table row than the build presented. The owner is ruling on an override | **owner** |
| ⚠⚠ **`12d-Q2`** | **§3.4 names no STRING for the third form.** It gives the meaning and leaves the copy open — the same family as `12c-Q5` | **`list not fully read`**; three alternatives weighed in `12d-build.md` §1.2. ⚠ **Changing it is NOT a one-line edit**: 2 sites in `gpu-panel.tsx`, 10 in `gpu-panel.test.tsx`, 2 in `serving-panel.test.tsx`, and **2 mutations** (`12d-GP1`, `12d-GP2`) anchor on the literal — so an anchor census plus a 359-mutation harness run | **owner** |
| **`12d-Q3`** | **Should the card name HOW MANY rows were refused?** §3.4 says *"the card says only that it cannot answer and why the question is open"* and names no count; §6.2 names none either | **No count.** ⚠ The recorded reason was rewritten by the reconcile: it had read *"§3.4 says keep it short"*, and that sentence is **not in `SPEC.md`** — it is the parent's own build handoff. A count answers *how much* we failed to read, which is neither half of what §3.4 licenses the card to say. `refused.length` reaches the panel, so adding it is free the day there is wording | **owner** |
| **`12d-Q4`** | **Should a list read in PART produce an event-log line?** §6.4's vocabulary is transitions of bands and of closed-vocabulary values; a change in *completeness* is neither | **Nothing added.** The `llama-env` refusal entry already moves `failingSourceCount` and, after 12c's fix, escalates in the log when what that source says changes | **owner** |
| **`12d-Q5`** | **A refused row may name an identity that ALSO appears on a row we read** (`12c-Q6`: the wire does not police duplicates). It bites on the poll after the good row disappears | **Held back anyway**, the conservative direction, and fixtured since the test phase. §9 does not say whether a subject read about *elsewhere in the same array* counts as accounted for, and guessing that it does is the direction that deletes an alarm | **owner** |
| ⚠ **`12d-Q6`** NEW | ⚠⚠ **THE TWO PANELS NOW CONTRADICT EACH OTHER IN PROSE, and 12d created it.** With every row refused, SERVING's headline reads *"no llama-server instances discovered"* — a positive claim about the machine — directly above two *"serving[N] was dropped: `port` did not validate"* sentences, and one panel away from two cards saying *"list not fully read"*. Before 12d the cards rendered `—` and made no claim, so the page did not contradict itself. Supersedes `12c-Q5`'s *"carried, unchanged"*, which describes it as smaller than it is | **Not built**, and not because it is expensive: `state.serving.read === 'partial'` is reachable from the panel and it is one line. It is **panel copy nobody has ruled on**, `MOCK.html` is the source for form, and neither the build nor the reconcile may edit those — invariant 7. The refusal notes do render underneath, so it is not silent | **owner**, one sentence of copy |
| ⚠ **`12d-Q7`** NEW | ⚠⚠ **STALENESS IS UNBOUNDED, AND 12d/RECONCILE WIDENED IT.** Nothing ages a stale condition out — `state.remembered` is rebuilt every poll and the only deletion is inside the retire branch — so a row the server never fixes keeps its alarm in the ledger, the dot and the count **forever**, which is the defect §9's ruling removed, reproduced for one subject by design. ⚠ The membership clause extends it: a row that stays in `serving[]` reporting `health: null`, or a card that stays in `gpus[]` with `tempC: null`, now keeps its last alarm for ever where before it was deleted | **Recorded, not changed.** §9 says a protected subject goes *stale, never retired*; `lib/conditions.ts` carries the reason (*"An expiry would be a clock that silently turns an alarm green"*); §6.5's stale mode shows the age of the reading behind it. The widening is the spec's own preference — *"An unobservable alarm is **unknown**, not resolved"* — chosen over deleting a live alarm. **Whether staleness needs a HORIZON is a §9 ruling** | **owner** |
...
**Open, with owners — FIFTY-SIX rows: ⚠⚠ 12d's seven (`12d-Q1`…`12d-Q7`, above), ⚠⚠ 12c's six (`12c-Q1`…`12c-Q6`, above), ⚠⚠ 12b's six (`12b-Q1`…`12b-Q6`, below), 12a's five (`12a-Q1`/`Q2`/`Q9`/`Q10`/`Q11`, below —
