...
⚠⚠ **A PARTIALLY-READ LIST SAYS SO ON THE CARD — ruled 2026-09-22 (12c-Q1).** When a `serving[]` row is refused, the GPU card's served-by line showed an **em dash** — and an em dash already means *this reading could not be taken*, which is what an unreadable `gpus` shows. Two different facts rendering identically is the collapse invariant 1 exists to prevent, one level up: *we discarded part of the list* is not *the unit's cards could not be read*. ⚠ And the `errors[]` entry explaining the refusal reaches **only the SERVING panel**, so the card carried a failure it could not account for.

**So a partial read gets its own form on the card** — distinct from the em dash and from a positive answer — saying that the list was incomplete rather than that the reading failed. It is a statement about *our* knowledge, not about the machine, and it must read that way. The full reason stays on SERVING beside the refused row; the card says only that it cannot answer and why the question is open.
...
|---|---|---|
| Aggregate status dot **and count** | **One reduction over each condition's `displaySeverity`** (§6.4), which is the debounced band after standing suppression — **with one refusal on top of it, added 2026-09-15: a `normal` reduction is painted as NO BAND while any §3.7 source failed to be read**, and the number of those sources is named in the text beside it (`● 18 sources unread`). ⚠ **`watch` and `alarm` are never lowered by this**, so a failing collector can never hide an alarm. A suppressed standing condition is neither red nor counted — its truth is named in its SAFETY row instead. The count is **omitted when zero**, so the header reads `● all healthy`, never `0 alarms`. **Paused/stale is a mode shown alongside it, never instead of it** | Reducing over `displaySeverity` is what keeps the dot, the count and the banner from ever disagreeing. A mode that hid the count would be a lying dashboard. ⚠ **The refusal exists because this row, as first written, was measured false in production**: a `daemon-reload` revoked the container's GPU access, `nvidia-smi` failed, and the header read `● all healthy` — a source that cannot be read produces no condition, so it contributed nothing to a reduction *over conditions*, and the summary described a half-blind machine as a well one. §6.6's law that an absent reading is not a healthy one, applied to the summary rather than to a cell |
...
| A condition whose subject stops being reported | **The reduction runs over every condition the session has confirmed, not only those in this poll.** A condition absent from a poll is **stale**: it keeps its last confirmed `displaySeverity`, keeps its "since", keeps counting toward the dot and the count, and shows the age of the reading behind it. It leaves the reduction only when **retired** — the collection that would have contained it was read successfully and it was not in it (`gpus: [{index:0}]` for GPU 1, `serving: []`). A collection that could **not** be read (`gpus: null`) retires nothing. ⚠⚠ **A PARTIAL READ RETIRES WHAT IT CAN — ruled 2026-09-22 (12c-Q2).** The first cut froze retirement for **every** instance whenever any row was refused, so an instance that genuinely left the machine kept its alarm in the count indefinitely — measured still there at 20 minutes — because one unrelated row failed validation. Safe, and unbounded. The rule is now per subject, and it turns on **whether the refused row's identity parsed**:

- **Identity parsed, another field failed** → we know exactly which subject that row was about. **Protect that identity only**; every other absent subject retires normally.
- ⚠ **Identity did NOT parse** → we do not know which subject the row would have named, so no subject can be shown absent. **Retirement is frozen for the whole enumeration**, as before — and that is not a special case to be optimised away, it is the honest answer to *we cannot tell who is missing*.

The `errors[]` entry names which row and why in both cases, and a protected subject goes **stale**, never retired.

⚠⚠ **NOR DOES A ROW WE OURSELVES DROPPED — added 2026-09-18, after this was measured deleting a live alarm.** §3.4's second 2026-09-17 ruling lets `wire.ts` refuse one bad `serving[]` row and render the rest, which leaves the array **shorter** — and a shorter array is exactly what retirement reads as. Measured: one malformed `port` refused two rows, and past §6.4's debounce a genuine **`alarm`** left the ledger, the dot and the count, with the header reading `normal`. **The collection was read successfully and the client discarded part of it, which is not the same as the server not reporting it.** So a row refused by validation makes its conditions **stale**, never retired, and the `errors[]` entry that names the refusal is what says why. Retirement requires that the *server* omitted the subject from a collection it could read. Staleness never raises a severity and never lowers one | An unobservable alarm is **unknown**, not resolved. Without this, a card at 90 °C whose `nvidia-smi` then fails takes the header from `● 1 alarm` to `● all healthy` with no log line — the dashboard turns green at the moment it loses the ability to look. The `null` ≠ `[]` distinction §3.1 already carries is exactly what separates *not read* from *not there* |
